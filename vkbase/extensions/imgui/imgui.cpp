...
{
    std::vector<VkCommandBuffer> cbIMGUI;
    ...
    static bool processPendingFonts();
...
////////////////////////////////////////
        ...
        //disable CTRL+TAB for window switching
        ...
        init_info.ImageCount = vkbase::imageCount;
...
////////////////////////////////////////////
        ...
        addDrawPrepareCallback([](uint32_t imageIndex)
        {
            renderIMGUI();
        });

        addRewriteBuffersCallback([](uint32_t bufferIndex)
        {
            rewriteIMGUICommandBuffer(bufferIndex);
        });
        ...
        addWriteMainBufferCallback([](VkCommandBuffer cb, uint32_t bufferIndex)
        {
            vkCmdExecuteCommands(cb, 1, &cbIMGUI[bufferIndex]);
        },-100);
    }
    ...
    void createIMGUICommandBuffers()
    {
        ...
        allocInfo.commandBufferCount = (uint32_t) cbIMGUI.size();

        //create command buffers
        if (vkAllocateCommandBuffers(device, &allocInfo, cbIMGUI.data()) != VK_SUCCESS)
        {
    ...
    struct FontRequest { std::string name; int size; };
    static std::vector<FontRequest> pending_font_requests;
    uint8_t font_size;
    std::string font;
    Event<> onUiEvent;
    static float defaultFontSize = 16.0f;
    static std::string defaultFontName = "";
    void renderIMGUI()
    {
        if (fonts.find(defaultFontName)==fonts.end() || fonts.at(defaultFontName).find((int)defaultFontSize)==fonts[""].end())
        {
            pending_font_requests.push_back(FontRequest{defaultFontName, (int)defaultFontSize});
        }
        // Process any deferred font loads before building the new frame
        processPendingFonts();

        //After everything is written to command buffer we can start new Dear ImGui frame
        ImGui_ImplVulkan_NewFrame();
        newFrameIMGUIPlatformBackend();
        ImGui::NewFrame();
        pushFont(defaultFontName,defaultFontSize);
        onUiEvent.call();
        ImGui::PopFont();
        ...
        //Finish writing IMGUI commands and start rendering
        ImGui::Render();
    ...
    {
        VkCommandBuffer cb = cbIMGUI[imageIndex];
        ...
        //render commands
        {
            ...
            ImGui_ImplVulkan_RenderDrawData(imguiDrawData, cb);
        }
    ...
    bool showDebugWindow()
    {
        ...
        static std::string gpuName = getDeviceName(physicalDevice);

        if(ImGui::Begin("Debug Window", &showDebug))
        {
            ImGui::Text("FPS: %.1f", io.Framerate);
            ImGui::Text("Frame time: %.3f ms", lastFrameTime * 1000);
            ImGui::Text("GPU: %s", gpuName.c_str());
            ImGui::Text("Resolution: %dx%d", extent.width, extent.height);
            ImGui::Text("Images count in swap chain: %d", imageCount);
        }
        ImGui::End();
        return showDebug;
    }
    ...
    {
        return ImGui_ImplVulkan_GetBackendData()->DescriptorSetLayout;
    }


    void dockWindow(const char* A, const char* B, float proportion)
    {
        ImGuiWindow* windowA = ImGui::FindWindowByName(A);
        ImVec2 APos = windowA->Pos;
        ImVec2 ASize = windowA->Size;


        ImGuiID parent_node=ImGui::DockBuilderAddNode();
        ImGui::DockBuilderSetNodePos(parent_node, APos);
        ImGui::DockBuilderSetNodeSize(parent_node, ASize);
        ImGuiID nodeA;
        ImGuiID nodeB;
    ...
    int addOnUiCallback(const std::function<void()>& callback,int priority)
    {
        return onUiEvent.addCallback(callback, priority);
    }
    void removeOnUiCallback(int id)
    {
        onUiEvent.removeCallback(id);
    }
    ...
    static bool processPendingFonts()
    {
        if(pending_font_requests.empty())
            return false;

        //remove all unused fonts
        for (auto& [font_name, f]:fonts)
        {
            auto it=f.begin();
            while (it!=f.end())
            {
        ...
        //add new fonts
        for(const auto &req : pending_font_requests)
        {
            fonts[req.name][req.size]=Font();
        }


        ImGuiIO &io = ImGui::GetIO();
        io.Fonts->Clear();
        for (auto& [font_name, fonts_by_name]:fonts)
        {
            for (auto& [font_size, f]:fonts_by_name)
            {
                ...
                if(a_it != vkbase::assets.end())
                {
                    ...
                    added = io.Fonts->AddFontFromMemoryTTF((void*)font_binary.data(), (int)font_binary.size(), font_size, &cfg);
                }
                else
                {
                    ImFontConfig cfg{};
                    ...
                    added = io.Fonts->AddFontDefault(&cfg);
                }
                if(!added)
                {
                    std::cerr << "[processPendingFonts] Failed AddFontFromMemoryTTF: " << asset_key << "\n";
                    continue;
                }
                fonts[font_name][font_size]=Font{.font= added,.is_used = false};
            }
        }
        ...
        if(!ImGui_ImplVulkan_CreateFontsTexture())
            std::cerr << "[processPendingFonts] Failed to rebuild font texture after additions\n";

        pending_font_requests.clear();
        return true;
    }
    ...
    void pushFont(const std::string& font_name, int size)
    {
        ...
        if(auto f_it = fonts.find(font_name); f_it != fonts.end())
        {
            auto &size_map = f_it->second;
            if(auto s_it = size_map.find(size); s_it != size_map.end() && s_it->second.font)
            {
                ImGui::PushFont(s_it->second.font);
                s_it->second.is_used=true;
                return;
            }
        }

        pending_font_requests.push_back(FontRequest{font_name,size});
        // Push default for now (will be replaced next frame when processed)
