...
float4 main(float4 position): SV_TARGET
{
    ...
    float2 dr = float2(doubleNoise(float2(t,0)), doubleNoise(float2(0,t + 0.5)));
    float2 dg =float2(doubleNoise(float2(t + 100.1111,0)), doubleNoise(float2(0,t - 100.4444)));
    float2 db =float2(doubleNoise(float2(t + 1000.22222,0)), doubleNoise(float2(0,t + 1000.555555)));

    applyColor(float4(doubleNoise(uv + dr * 10),doubleNoise(uv + dg * 10), doubleNoise(uv + db * 10), 1));

    circle(dr, 0.1, float4(1, 0, 0, 0.9));
    circle(dg, 0.1, float4(0,1, 0, 0.9));
    circle(db, 0.1, float4(0, 0,1, 0.9));
    IdentityBox();
