...
#undef RESOURCE
      ...
      input_files.emplace_back(
              ...
              {
                  arg.str(),
                  (current_fshader_stage == shaderc_glsl_infer_from_source ? glslc::DeduceDefaultShaderKindFromFileName(arg): current_fshader_stage),
                  language,
                  current_entry_point_name
              }
          );
  ...
  if (!compiler.ValidateOptions(input_files.size()))
      return 1;
